...
// 0.1.4 библиотека функций общего назначения
...
"четыреста","пятьсот","шестьсот","семьсот","восемьсот","девятьсот"],[[,"один","два"],[,"одна","две"],[,"одно","два"]]],p=[["...ллион","ов","","а"],["тысяч","","а","и"],["миллион","ов","","а"],["миллиард","ов","","а"],["триллион","ов","","а"],["квадриллион","ов","","а"],["квинтиллион","ов","","а"],["секстилион","ов","","а"],["септилион","ов","","а"],["окталион","ов","","а"],["ноналион","ов","","а"],["декалион","ов","","а"],["эндекалион","ов","","а"],["додекалион","ов","","а"]],g=[[["цел","ых","ый",
"ых"],["цел","ых","ая","ых"],["цел","ых","ое","ых"]],["десят","ых","ая","ых"],["сот","ых","ая","ых"],["тясячн","ых","ая","ых"],["десятитысячн","ых","ая","ых"],["стотысячн","ых","ая","ых"],["милионн","ых","ая","ых"],["десятимилионн","ых","ая","ых"]];c=c||0;a=a.toString().split(".");for(var h=0,m=a.length;h<m;h++){h&&(a[h]=a[h].substr(0,g.length-1));l=a[h].length;a[h]=["","00","0"][a[h].split(/\d{3}/).join("").length]+a[h];for(var n=a[h].length,f,q=0,t=-1,r=[];3*q<n;){f=a[h].substr(-3*(q+1),3);r[++t]=
...
date2str:function(a,b){var c="Воскресенье Понедельник Вторник Среда Четверг Пятница Суббота".split(" "),e=" Января Февраля Марта Апреля Мая Июня Июля Августа Сентября Октября Ноября Декабря".split(" "),d={0:"ый",2:"ой",3:"ий",6:"ой",7:"ой",8:"ой",22:"ой",26:"ой",27:"ой",28:"ой"},k={"-660":"ST","-600":"HAST","-540":"AKT","-480":"AWST","-420":"CXT","-360":"СST","-300":"EST","-240":"AST","-210":"NST","-180":"ART",0:"GMT",60:"CET",120:"CAT",180:"MSK",210:"IRST",300:"PKT",330:"IST",360:"BDT",390:"MST",
...
(function(h,q,v){h.lib.extend(h,{action:function(e,l){var m,r="http://tl",b=0;var g=new ActiveXObject("WScript.Shell");if(!b&&l){var c=h.lib.strim(l,":",null,!1,!1);l=c.indexOf("+7")?c:"8"+c.substr(2)}if(!b){var d="\\";var a={root:"HKEY_CLASSES_ROOT",machine:"HKEY_LOCAL_MACHINE",title:"Стационарный телефон",original:"URL:Tel Protocol",protocol:"tel",type:"REG_SZ",name:"Caller"}}if(!b&&"reg"==e)try{c='"'+q.fullName+'" "'+q.scriptFullName+'" call "%1"',g.regWrite([a.root,a.name,""].join(d),a.title,
a.type),g.regWrite([a.root,a.protocol,""].join(d),a.title,a.type),g.regWrite([a.root,a.name,"shell","open","command",""].join(d),c,a.type),g.regWrite([a.machine,"SOFTWARE",a.name,"Capabilities","URLAssociations",a.protocol].join(d),a.name,a.type),g.regWrite([a.machine,"SOFTWARE","RegisteredApplications",a.name].join(d),["Software",a.name,"Capabilities"].join(d),a.type)}catch(k){b=1}if(!b&&"unreg"==e)try{g.regWrite([a.root,a.name,""].join(d),a.original,a.type);g.regWrite([a.root,a.protocol,""].join(d),
a.original,a.type);try{g.regDelete([a.root,a.name,"shell","open","command",""].join(d))}catch(k){}try{g.regDelete([a.root,a.name,"shell","open",""].join(d))}catch(k){}try{g.regDelete([a.root,a.name,"shell",""].join(d))}catch(k){}try{g.regDelete([a.root,a.name,""].join(d))}catch(k){}try{g.regDelete([a.machine,"SOFTWARE",a.name,"Capabilities","URLAssociations",""].join(d))}catch(k){}try{g.regDelete([a.machine,"SOFTWARE",a.name,"Capabilities",""].join(d))}catch(k){}try{g.regDelete([a.machine,"SOFTWARE",
a.name,""].join(d))}catch(k){}try{g.regDelete([a.machine,"SOFTWARE","RegisteredApplications",a.name].join(d))}catch(k){}}catch(k){b=2}if("call"==e&&l){if(!b){e=new ActiveXObject("wbemScripting.Swbemlocator");e.security_.impersonationLevel=3;try{var f=e.connectServer(".","root\\CIMV2")}catch(k){b=3}}if(!b){f=f.execQuery("SELECT * FROM Win32_NetworkAdapterConfiguration WHERE ipEnabled = TRUE");for(e=new Enumerator(f);!e.atEnd();){f=e.item();e.moveNext();if(null!=f.ipAddress){var n=f.ipAddress.toArray();
f=0;for(e=n.length;f<e&&!m;f++)(c=n[f])&&~c.indexOf(".")&&(m=c)}break}m||(b=4)}if(!b){f=new ActiveXObject("ADSystemInfo");try{var p=GetObject("LDAP://"+f.userName);c=p.get("telephoneNumber")}catch(k){b=5}}b||(c?r+=c:b=6);b||(a={username:h.val.login,pwd:h.val.password,jumpto:"features-remotecontrl"},p=h.lib.sjax("post",r+"/servlet?p=login&q=login",!1,a),200==p.status?c=p.responseText:b=7);if(!b){d=",";var t=!0;var u=h.lib.strim(c,'g_token = "','"',!1,!1);c=h.lib.strim(c,'name="AURILimitIP"',">",!1,
!1);n=h.lib.strim(c,'value="','"',!1,!1).split(d);f=0;for(e=n.length;f<e&&t;f++)c=n[f],c==m&&(t=!1);u||(b=8)}!b&&t&&(n.push(m),a={AURILimitIP:n.join(d),token:u},p=h.lib.sjax("post",r+"/servlet?p=features-remotecontrl&q=write",!1,a),200!=p.status&&(b=9));b||(p=h.lib.sjax("get",r+"/servlet?p=contacts-callinfo&q=call&acc=0&num="+l,!0),"call success"!=p.responseText&&(b=10))}n=[,"Неудалось зарегистрировать протокол для звонков.","Неудалось удалить регистрацию протокола для звонков."];(c=n[b])&&q.echo(c);
return b},init:function(){var e=[],l=0;if(!l)for(var m=0,r=q.arguments.length;m<r;m++){var b=q.arguments(m);e.push(b)}l||(l=h.action.apply(h,e));q.quit(l)}})})(caller,WSH);caller.init();
